...
import { i as g } from "chunk-VJ7UYMJI.mjs";
...
var h = g("message-channel-connection");
var p = class {
  constructor(e) {
    this.options = e;
  }
  ...
  shouldHandleRendezvous(e, n) {
    ...
    return (b(t, e.data.channelConfig) || l(!1, "Channel configuration mismatch"), !0);
  }
  setupMessageChannel(e) {
    ...
    return (
      ...
      (n.port1.onmessage = (t) => {
        t.data.type === "reply" &&
          (this.isConnected() || (l(t.ports[0], "Missing port"), this.setup(n.port1, t.ports[0])));
      }),
      n
    );
  }
  sendConnectionMessage(e, n, t, s) {
    ...
    e.postMessage(o, n.url, [t.port2]);
  }
  trySendMessage(e, n, t, s, o) {
    try {
      this.sendConnectionMessage(e, n, this.setupMessageChannel(o), t);
    } catch (r) {
      (r instanceof DOMException &&
        r.name === "DataCloneError" &&
        (s
          ? h.errorOncePerMinute(
              "MessagePort was neutered on retry attempt. Check your browser extensions. Error:",
              r
            )
          : (h.debug("MessagePort was neutered, creating new channel and retrying:", r),
            this.trySendMessage(e, n, t, !0, o))),
        h.error("Failed to send connection message:", r));
    }
  }
  setupActiveConnection(e) {
    (h.debug("setupActiveConnection", { options: e }),
      l(e.target, "Target window is required for active connection"));
    let n = e.target,
      t = [],
      s = () => {
        if (this.isConnected()) return;
        let r = Math.floor(Math.random() * Number.MAX_SAFE_INTEGER);
        ((this.localWeight = r), this.trySendMessage(n, e, r, !1, t));
      },
      o = setInterval(s, 500);
    return (
      s(),
      {
        cleanup: () => {
          clearInterval(o);
          for (let r of t) ((r.onmessage = null), r.close());
          t.length = 0;
        },
      }
    );
  ...
  setupPassiveConnection(e) {
    let n = (t) => {
      u({ source: t.source, target: e.target, origin: t.origin, url: e.url }) &&
...
var z = g("post-message-transport"),
  m = class {
    constructor(e, n, t) {
      this.controllerId = e;
      this.handleAllMessages = n;
      (t?.addEventListener(
        "abort",
        () => {
          this.target &&
        ...
        (this.messageHandler = (s) => {
          ...
          u({ source: s.source, target: this.target, origin: s.origin, url: this.origin }) &&
  ...
  return new Proxy(
    {},
    {
      get: (t, s) => {
        ...
        let r = (...i) => a.sendOnewayMessage(e, s, i);
        return (n.set(s, r), r);
      },
    }
  );
...
var d = g("message-channel-controller");
var M = class {
    constructor(e, n) {
      this.channel = e;
      this.object = n;
  ...
  x = class {
    constructor(e, n) {
      this.channelInterface = e;
      this.abortSignal = n;
      ((this.id = Math.floor(Math.random() * Number.MAX_SAFE_INTEGER)),
        (this.messagePortTransport = new f(this.handleAllMessages, n)),
        (this.postMessageTransport = new m(this.id, this.handleAllMessages, n)),
        (this.connectionManager = new p(this)));
    }
    channelInterface;
    abortSignal;
    id;
    ...
    handleAllMessages = (e) => {
      if (!P(e.data.type)) return;
      let n = e.data;
      switch (n.type) {
        case "result": {
          let t = this.pendingCalls.get(n.seq);
          if (!t) {
            d.warn("No pending call found for seq:", n.seq, n);
            return;
          }
    ...
    handleInvokeMessage(e) {
      d.trace("handleChannelMessage", e);
      ...
      try {
        ...
        let o = s.apply(t.object, e.args);
        if (e.type === "invokeOneWay") return;
        if (T(o)) {
          o.then((r) => {
            n.send({ type: "result", seq: e.seq, result: r });
          }).catch((r) => {
            (d.error("method", e.method, "promise threw an error:", r),
              n.send({ type: "error", seq: e.seq, error: r }));
          });
          return;
        }
        n.send({ type: "result", seq: e.seq, result: o });
      } catch (t) {
        if ((d.error("Method", e.method, "threw an error:", t), e.type === "invokeOneWay")) return;
        n.send({ type: "error", seq: e.seq, error: t });
      }
    }
    debugSendMessage = void 0;
    sendOnewayMessage(e, n, t) {
      (d.trace("oneway message:", e, n, t), this.messageCounter++);
    ...
    async sendMessage(e, n, t) {
      (d.trace("message:", e, n, t), this.messageCounter++);
      ...
      return (
        ...
        this.debugSendMessage
          ...
          : new Promise((o, r) => {
              this.pendingCalls.set(this.messageCounter, { resolve: o, reject: r });
            })
      );
    }
    ...
    setupChannelAsLocal(e, n) {
      ...
      this.localChannels.set(e.id, new M(e, n));
    }
    setupChannelAsRemote(e) {
      ...
      return (n || ((n = new y(e, w(this, e.id))), this.remoteChannels.set(e.id, n)), n.proxy);
    }
    setLocal(e, n) {
...
var R = class {
  constructor(e = "message-channel") {
    this.communicationMethod = e;
  }
  communicationMethod;
  _id;
  get id() {
    return (l(this._id, "Channel defined outside of ChannelInterface"), this._id);
  }
  setId(e) {
    (l(!this._id, "Cannot set id twice."), (this._id = e));
  }
};
var S = class {
  constructor(e, n) {
...
//# sourceMappingURL=chunk-FUMDHGSM.mjs.map
