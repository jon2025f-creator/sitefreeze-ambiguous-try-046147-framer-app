import { b as C, q as m, r as u, s as F } from "chunk-DD7EZN5V.mjs";
import { f as P } from "chunk-OTBEGHUQ.mjs";
import { h as j } from "chunk-LA34HORX.mjs";
import { i as A } from "chunk-VJ7UYMJI.mjs";
...
function B(s) {
  ...
  return function (o) {
    ...
    c === i ? (r = void 0) : c !== 0 && ((r = r.subarray(c)), (t -= c));
  };
}
function I(s, r, t) {
  let e = $(),
...
function T(s, r) {
  ...
  return new Promise((l, D) => {
    ...
    let O = _,
      b = 0;
    function E() {
      (document.removeEventListener("visibilitychange", L), window.clearTimeout(b), y.abort());
    }
    t?.addEventListener("abort", () => {
      (E(), l());
    });
    let H = d ?? window.fetch,
      N = n ?? z;
    async function S() {
      var k;
      y = new AbortController();
      try {
        let w = await H(s, Object.assign(Object.assign({}, h), { headers: p, signal: y.signal }));
        (await N(w),
          await V(
            w.body,
            B(
        ...
        if (!y.signal.aborted)
          try {
            let f = (k = i?.(w)) !== null && k !== void 0 ? k : O;
            (window.clearTimeout(b), (b = window.setTimeout(S, f)));
          } catch (f) {
            (E(), D(f));
          }
      }
...
function z(s) {
  ...
  if (!r?.startsWith(R)) throw new Error(`Expected content-type to be ${R}, Actual: ${r}`);
}
var M = A("fetcher"),
  G = (s, r) => {
    if (!r) return s;
    ...
    for (let e in r) {
      ...
      if (n !== void 0) {
        if (j(n)) {
          for (let a of n) a && t.searchParams.append(`${e}[]`, a.toString());
          continue;
        }
...
async function q(s, r) {
  ...
  let t = s.statusText,
    ...
    a;
  try {
    let { error: o, ref: i, data: c } = await s.json();
    (typeof o?.message == "string" && (t = o.message),
      typeof o?.code == "number" && (e = o.code),
      typeof i == "string" && (n = i),
      typeof c == "object" && c && (a = c));
  } catch {}
  return s.status === 401 &&
    (await m.discardAccessToken(!0), t.toLowerCase().includes("jwt expired") && r < 1)
    ? (M.reportError(
        new Error(
      ...
      { status: "retry" })
    : { status: "error", error: new C({ data: a, message: t, status: s.status, ref: n, code: e }) };
}
var v = class {
  constructor(r) {
    this.baseURL = r;
  ...
  fetchStream(r, t = {}, e = 0) {
    return this.doWithListeners(async () => {
      let { headers: n, onopen: a, onerror: o, ...i } = t,
  ...
  async getRaw(r, t, e) {
    return await this.fetch(G(`${this.baseURL}${r}`, t), {
      credentials: "include",
      redirect: "error",
      signal: e,
    });
  ...
  postStream(r, t, e) {
    if (this.readonly) throw Error("apiFetcher is set to readonly");
    let n = typeof e < "u",
      a = n ? { "Content-Type": "application/json", ...u } : u;
  ...
  async put(r, t, e) {
    if (this.readonly) throw Error("apiFetcher is set to readonly");
    let n = await this.putRaw(r, JSON.stringify(t), "application/json", e);
    return g(n) ? n.json() : n.text();
  }
  async patchRaw(r, t, e) {
    if (this.readonly) throw Error("apiFetcher is set to readonly");
    let a = typeof t < "u" && e ? { "Content-Type": e, ...u } : u;
    return await this.fetch(`${this.baseURL}${r}`, {
      method: "PATCH",
      credentials: "include",
      headers: a,
...
var pe = new v(x().api);
export { K as a, pe as b };
//# sourceMappingURL=chunk-CL6NHK4I.mjs.map
