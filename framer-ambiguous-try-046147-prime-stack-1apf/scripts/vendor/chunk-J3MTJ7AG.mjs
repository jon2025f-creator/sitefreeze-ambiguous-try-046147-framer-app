...
import { b as tj } from "chunk-54KK7FUJ.mjs";
import { V as vA, X as KA, Y as CA } from "chunk-YUW2K2TN.mjs";
import { Rb as C } from "chunk-2GRXI3ZY.mjs";
import { s as H } from "chunk-7ZCOVLUS.mjs";
import { a as AA } from "chunk-OOA4IV6W.mjs";
import { g as N } from "chunk-FYZXEFSV.mjs";
import {
  Hl as fA,
  Il as bA,
  Ir as OA,
...
import { b as SA } from "chunk-PHV34KT6.mjs";
import {
  Ba as O,
  Da as GA,
  Pe as pA,
...
import { c as h, d as HA } from "chunk-JGLE5AEB.mjs";
import { f as R } from "chunk-EFB2P3QE.mjs";
import { a as UA } from "chunk-GBWZWM2Q.mjs";
import { a as m } from "chunk-6TFWVVAP.mjs";
import { g as dA } from "chunk-ONZSSKY2.mjs";
import { b as c, k as RA, m as FA } from "chunk-LA34HORX.mjs";
import { b as d, c as D } from "chunk-4JY5UMT2.mjs";
import { b as q, h as M } from "chunk-KPMZENE5.mjs";
var kj = "https://app.framerstatic.com/framer_compiler_bg-UGK2KYHD.wasm";
var F = M("compiler");
async function e6() {
  await y();
}
...
async function t6({ localId: A, name: j, source: t, includeSourceMap: r = !0 }) {
  let e = await y();
  try {
    let {
        code: k,
...
window.$RefreshSig$ = prevRefreshSig;`,
      { moduleCode: n, annotations: l, exportedNames: $ } = oj(i, a, !0);
    return {
      code: n,
      sourceMap: o ?? void 0,
...
async function k6({
  ...
  try {
    let {
      ...
      { moduleCode: b, annotations: I, exportedNames: B } = oj(l, s, e);
    return {
      code: b,
      sourceMap: $ ?? void 0,
    ...
    let $ = `Failed to compile module ${A}: ${l}`;
    return (
      F.debug($, "(enable trace logging to see full source)"),
      F.trace(j),
      A.startsWith("codeFile/") ||
        (F.reportCriticalError(new Error($, { cause: l }), { moduleName: A, moduleSource: j }),
        R("application_error", { message: $, area: "compiler" })),
      aj(l)
    );
  } finally {
...
async function y() {
  if (!w) {
    let A = new Worker(SA("./modulesCompilerWorker.js")),
      j = OA(A);
    ((w = j
      .initWithPathToWasm(kj)
      .then(() => ({ transformSync: j.transformSync, analyzeImports: j.analyzeImports }))),
      w.catch(() => {
        if (UA() || !dA.isProduction)
...
function d6(A, j, t, r, { shouldOpenPage: e = !0, renamedIds: k = new f() } = {}) {
  let a = QA(A, j, t, "duplicate", {
    enterIsolation: !1,
    preferredName: r,
  ...
  return (
    A.stores.persistedUserDefaults.newContentAsDraft &&
      a.set({ isDraft: !0, duplicatedFrom: _A(a.duplicatedFrom, j.id) }),
...
async function R6(A, j, t) {
  let r = j.isLoaded() ? j : await j.load();
  if (r) return Ij(A, r, t);
}
function nj(A, j, t = !0) {
...
async function uj(A, j, t) {
  ...
  let o;
  if (k.getDataTree()) {
    for (let i of k.query().whereClass(MA).iterate())
...
async function U(A, j, t) {
  return (await uj(A, j, t)) ? (A.stores.pluginStore.openPlugin(t, A, { mode: "canvas" }), !0) : !1;
}
function X(A, j) {
  let t = A.componentForIdentifier(j);
  if (t) return t;
  let r = O(j);
  return !u(r) || !cA(r) ? null : A.componentForIdentifier(BA(r));
}
var _j = 0;
async function V(A, j, t = _j) {
  if (t === 0) {
    let e = O(j),
...
async function $j(
  ...
  let o = t.module.baseURL + t.module.files.source,
    ...
    [l, $] = await Promise.all([ij(n), A.stores.modulesStore.getModuleDependencies(j)]);
  if (l.ok) {
    let B = new Map();
    for (let E of l.value.relative) {
      let g = hA(E.specifier, `${t.module.type}/`);
      if (!g) {
    ...
    for (let E of $.imports) {
      ...
      for (let sA of P) {
        p ||
          (p = n.split(`
`));
        let lA = sA.specifierLine;
        p[lA] = p[lA].replace(JSON.stringify(sA.specifier), JSON.stringify(E.importURL));
      }
    }
    p &&
      (n = p.join(`
...
var sj = { canvasComponent: zA, codeFile: $j };
function rA(A) {
  return c(A) ? A in sj : !1;
}
async function kA(
  A,
  {
  ...
  if (!FA(s))
    return !c(s) && v(s)
      ? { ...n, codeComponentIdentifier: s.instanceIdentifier, node: s }
      : { ...n, codeComponentIdentifier: s, node: void 0 };
}
var tA = new Set();
async function aA(A, j) {
  ...
  if (r && a !== "autoUnlink" && (!k || tj.isOn("openPrimaryForBuiltInModules"))) {
...
async function lj(A, j) {
  ...
  if (!o) return;
  await LA(A.componentLoader, [o], A.stores.modulesStore, "component-unlinking");
        ...
        (A.tree
          ...
          .forEach(($) => {
            if (
              ...
              n.id === $.id ||
              !S($) ||
              $.codeComponentIdentifier !== t.codeComponentIdentifier ||
              mA($) ||
              !ZA($, { allowLockedNodes: !0 })
            )
              return;
            let s = A.cloneNode(n, !1, !1);
            Aj(A, [$], s);
          }),
        !!e)
      )
...
function be({ stackElement: A, tree: j }) {
  if (!A) return null;
...
function Oj({ treeStore: A, stackElement: j }) {
  if (!j) return null;
...
function ge({ treeStore: A, stackElement: j }) {
  return !j || !nA(j) ? null : Oj({ treeStore: A, stackElement: j });
}
function Oe(A) {
  ...
  if (a) {
    ...
    if (e.type === "existing")
      return e.itemId === a.id
        ? r
        : (d(r.length === 1, "Must dismiss overlay stack before changing selection"), [Ej(A, a)]);
...
function Sj(A, j) {
  ...
  for (let r = 0; r < j.length; r++) {
    ...
    let k = t.at(-1) ?? null;
    if (!hj(A, e, k) || (r > 0 && nA(e) && !e.saveAction)) break;
    t.push(e);
  }
  return t;
}
...
function z(A, j) {
  return NA(j, { currentUserId: A.stores.sessionStore.user.id });
}
function W(A, j) {
...
function Gj(A, j, t, r) {
  j.stores.canvasStore.invalidateTransformUntilRendered(() => {
    ...
    e &&
      ...
      j.stores.canvasStore.zoomToCenter(fA(j.tree, e), { animated: !1, maxZoom: 1 }));
  });
}
function Pj(A, j) {
  ...
  return (r && t === "block") || uA(e, "framerDisableUnlink");
}
function dj(A, j, t, r) {
  let e = O(A),
    k = x(j, "canDesign"),
  ...
  if (u(e) && e.type === "shader") {
    let $ = j.stores.modulesStore.getModuleEntryByLocalId(e.localId)?.name;
    return $
      ? [
          {
  ...
  if (EA(e) && e.kind === "externalModuleExport") {
        ...
        : [
            {
              label: "Edit",
              accelerator: a ? L.fakeSelectChildren : void 0,
              click: () => Bj(j, e.moduleId, r.node?.id),
              enabled: !k,
            },
          ];
    let $ = j.stores.insertSidebarStore.getEditBehavior(e.moduleId),
      s = j.stores.treeStore.tree.getNode(e.moduleId),
      I = r.externalModuleType === "canvasComponent" && s?.ownerId === "1h0OTH9KlHTd4pyc9CLt";
    if (Pj(j, e)) return m();
        ...
        ? [
            {
              ...
              accelerator: a ? L.fakeSelectChildren : void 0,
              click: () => {
                lj(j, { identifier: e, moduleType: E, instance: B, enterIsolation: !0 }).catch(q);
              },
              enabled: !k,
            },
          ]
  ...
  return [
    ...(n && YA(j.stores.modulesStore, e) && !k && u(A)
      ? [
          {
            label: WA(n),
            click: () => {
...
async function Dj(A, j, t, r) {
  ...
  return v(a)
    ? j.scheduler.processWhenReadyAsync(() => {
        let o = ej.detachSmartComponentOrWebPageInstance(j, A.draftOrCurrent(), a, e);
...
function Tj(
  ...
  let k = TA(j.tree, A);
  return dj(A.codeComponentIdentifier, j, k, {
    keepHistory: !0,
    node: A,
    withUnlinkModal: t,
...
function E7(A, j, t, r) {
  for (let e of A.variables) {
    if (e.type === "divider" || !DA(e)) continue;
    ...
    if ((!k || !c(k.value) || !k.value.length) && e.associatedStringVariable) {
      ...
      if (o && c(o.value)) {
        let i = Fj(o.value, (n) => rj(t.get(e.id), n, j.id));
        if (i.length)
          return (
...
function qj({ collectionId: A, engine: j, saveAction: t, source: r }) {
  ...
  (a && a.id !== e.id && d(t, "saveAction is required when creating a nested private draft item"),
    Mj({ collection: e, engine: j, saveAction: t }),
    R("collection_record_create", { source: r }));
}
function Mj({ collection: A, engine: j, saveAction: t }) {
  let r = vj(j, A, 0);
...
function vj(A, j, t) {
  let r = A.stores.sessionStore.user.id,
    e = Kj(j, r);
  if (e) return e;
  let k = CA.create({ privateToUserId: r, ...vA(A.tree.root.locales ?? m()) });
  return (A.stores.scopeStore.insertNode(k, j.id, t), xj(j, k), cj(A) && k.set({ isDraft: !0 }), k);
}
function Kj(A, j) {
  let t,
    r = 0;
...
//# sourceMappingURL=chunk-J3MTJ7AG.mjs.map
