...
import { g as oe } from "chunk-E2L4WW4T.mjs";
  ...
  var nt =
    ...
    function (r, e, t, n) {
      ...
      return new (t || (t = Promise))(function (o, a) {
        ...
        function f(h) {
          h.done ? o(h.value) : s(h.value).then(c, u);
        }
...
var v = class r extends Error {
    ...
    constructor({
      ...
      (super(),
        ...
        (this.skipSentry = !!c),
        (this.cause = u),
        s !== void 0 ? (this.isTemporary = s) : a && (this.isTemporary = a !== C.BadRequest));
    }
    toString() {
      return this.message;
    }
...
function ve(r, e, t) {
  let n = me(),
    s = new TextDecoder();
  return function (a, c) {
    if (a.length === 0) (t?.(n), (n = me()));
    else if (c > 0) {
      let u = s.decode(a.subarray(0, c)),
        f = c + (a[c + 1] === 32 ? 2 : 1),
        h = s.decode(a.subarray(f));
      switch (u) {
        case "data":
          n.data = n.data
            ? n.data +
...
function Qe(r, e) {
  ...
  return (n, s) => t.then((o) => s(o.transaction(e, n).objectStore(e)));
}
var K;
function b() {
  ...
  W = class {
    ...
    async refreshAccessTokenAndScheduleNextRefresh() {
      ...
      let t = await e.json(),
        ...
        s = le(n),
        o = new R(s.scopes);
      if (!this.isEmbeddedCrossOrigin && !this.isValidAccessToken(s.scope, o)) {
        (await this.discardAccessToken(),
          l.reportError(
            `Received access token has insufficient scopes. Wanted: ${J.valueOf()}, got: ${o.valueOf()}`,
            void 0,
            void 0,
            !0
          ),
      ...
      if (
        (t.expiresInSeconds
          ? ((a = Date.now() + t.expiresInSeconds * 1e3),
    ...
    async loadAccessTokenFromStorage() {
      ...
      try {
        ...
        let { accessToken: n, expiresAt: s } = t,
          o = new Date(s);
        return new Date() >= Y(o) ? null : { accessToken: n, expiry: o, storageKey: e };
      } catch (t) {
...
var Pt = T("fetcher"),
  Ot = (r, e) => {
    ...
    for (let n in e) {
      ...
      if (s !== void 0) {
        if (ae(s)) {
          for (let o of s) o && t.searchParams.append(`${n}[]`, o.toString());
          continue;
        }
...
async function je(r, e) {
  ...
  let t = r.statusText,
    ...
    o;
  try {
    let { error: a, ref: c, data: u } = await r.json();
    (typeof a?.message == "string" && (t = a.message),
      typeof a?.code == "number" && (n = a.code),
      typeof c == "string" && (s = c),
      typeof u == "object" && u && (o = u));
  } catch {}
  return r.status === 401 &&
    ...
    ? (Pt.reportError(
      ...
      { status: "retry" })
    : { status: "error", error: new v({ data: o, message: t, status: r.status, ref: s, code: n }) };
}
var B = class {
  constructor(e) {
    this.baseURL = e;
  ...
  fetch(e, t, n = 0) {
    return this.doWithListeners(async () => {
      let s = await this.accessTokenRefresher.withAuthorizationHeader(t || {}),
        o = await fetch(e, s).catch((c) => {
          throw Ct(c) ? c : Ve(c);
        }),
        a = await je(o, n);
      if (a.status === "error") throw a.error;
      return a.status === "retry" ? this.fetch(e, t, n + 1) : a.response;
    });
  }
  fetchStream(e, t = {}, n = 0) {
    return this.doWithListeners(async () => {
      let { headers: s, onopen: o, onerror: a, ...c } = t,
  ...
  async get(e, t, n, s) {
    let o = await this.getRaw(e, t, n, s);
    return _(o) ? o.json() : o.text();
  }
  ...
  postStream(e, t, n) {
    if (this.readonly) throw Error("apiFetcher is set to readonly");
    let s = typeof n < "u",
      o = s ? { "Content-Type": "application/json", ...m } : m;
...
//# sourceMappingURL=chunk-L36XFPVD.mjs.map
