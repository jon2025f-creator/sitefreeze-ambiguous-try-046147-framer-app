...
import { G as Te, f as Ce, i as Se, j as je } from "chunk-TALDZZ5D.mjs";
import { Ma as Ue, qa as Le } from "chunk-MF4EYFTI.mjs";
import { a as se } from "chunk-V4X34CXT.mjs";
import { B } from "chunk-WXI7GY67.mjs";
import { c as Ie } from "chunk-KCWQCVT5.mjs";
import { R as Me } from "chunk-CHXVQ5AG.mjs";
import { Wi as W, ci as ke } from "chunk-N56G23KA.mjs";
import { c as qe, d as xe } from "chunk-VPOACUZ4.mjs";
import { Nm as he, Qm as ye, cn as V } from "chunk-XFUU5W2C.mjs";
import {
  Ed as F,
  Pc as le,
  Wd as pe,
...
import { a as Be } from "chunk-TJ4P5JRV.mjs";
import { Z as N } from "chunk-WENOIAEG.mjs";
import { d as Ve } from "chunk-IUGMOHFE.mjs";
import { a as ve } from "chunk-K6L5GVTR.mjs";
import { a as we } from "chunk-UHN2KCK2.mjs";
import { q as Ae, u as r } from "chunk-L36XFPVD.mjs";
...
var us = M.default.memo(function ({
  ...
  let u = ue(),
    K = (0, M.useMemo)(() => _ ?? [], [_]),
    Je = (0, M.useMemo)(() => $e(t || de()), [t]),
    { value: f } = s,
    p = I(f) || b(f) ? f : ne,
    P = (d) => {
      (o(t, (g) => ({ ...g, value: d }), K), Re(p, d));
    },
    U = (d) => {
    ...
    Y = u.scheduler.wrapHandler((d) => {
      (P(d), Me.navigation.presentPopout(z(t)));
    }),
...
function ct(n, e) {
  ...
  if (!F(c)) return;
  let l = V(he, i);
  if (c.id === l) return (0, m.jsx)(je, {});
  let A = V(ye, i);
  if (c.id === A) return (0, m.jsx)(Se, {});
}
function O(n) {
...
var h = te("useAPI"),
  H = class {
    ...
    async pollProject({ intervalMillis: e, attempts: t, stopCondition: s }) {
      let o = await Ue(this.projectId, { intervalMillis: e, attempts: t, stopCondition: s });
      return (
        o.status === 0 &&
    ...
    async updateProject(e, t = !0) {
      ...
      try {
        (await Ve(this.projectId, e), this.getProject(), this.notifyProjectChange("metadata"));
      } catch (s) {
    ...
    async uploadAssetByURL(e, t = k) {
      let s = await Ke(`/web/projects/${this.projectId}/assets/fetch`, e, "image", t);
          ...
          s.map(async (i) => {
            ...
            if (B && window.CompressionStream) {
              let A = new window.CompressionStream("gzip"),
                $ = l.stream();
              l = await new Response($.pipeThrough(A)).blob();
...
//# sourceMappingURL=chunk-MACDOBE5.mjs.map
