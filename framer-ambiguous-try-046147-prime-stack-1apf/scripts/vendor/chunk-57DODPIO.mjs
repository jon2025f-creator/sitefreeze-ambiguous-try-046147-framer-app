...
var t = class {
  ...
  setVisible(e, d) {
    s(this.environment === "sandbox", "Setting data is only allowed in the sandbox.");
  ...
  import(e) {
    s(this.environment === "editor", "Importing data is only allowed in the editor.");
    let d = new Set();
    for (let r of e.addedRenderIds) (this.visibleRenderIds.add(r), d.add(r));
    for (let r of e.removedRenderIds) (this.visibleRenderIds.delete(r), d.add(r));
    return d;
  }
  getVisible(e) {
  ...
  export() {
    ...
    let e = Array.from(this.addedRenderIds),
      d = Array.from(this.removedRenderIds);
    return (
      this.addedRenderIds.clear(),
      this.removedRenderIds.clear(),
      { addedRenderIds: e, removedRenderIds: d }
    );
  }
};
export { t as a };
